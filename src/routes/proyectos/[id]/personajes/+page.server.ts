...
export const actions: Actions = {
	create: async ({ params, request }) => {
		...
		const [{ minPos }] = await db
			...
			.from(personajes)
			.where(eq(personajes.proyectoId, proyectoId));
		...
		try {
			...
			if (msg.includes('UNIQUE') || msg.includes('unique')) {
				return fail(400, { error: 'Ya existe un personaje con ese nombre en este proyecto' });
			}
			return fail(500, { error: 'No se pudo crear el personaje' });
		}
		...
		db.transaction((tx) => {
			for (let i = 0; i < ids.length; i++) {
				tx.update(personajes)
					.set({ position: i })
					.where(and(eq(personajes.id, ids[i]), eq(personajes.proyectoId, proyectoId)))
					.run();
			}
		});
