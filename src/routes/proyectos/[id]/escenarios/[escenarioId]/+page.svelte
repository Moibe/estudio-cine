<script lang="ts">
	...
	import { ArrowLeft, Check, MapPin, Pencil, X } from '@lucide/svelte';
	import type { PageProps } from './$types';

	let { data }: PageProps = $props();

	const escenasDelEscenario = $derived.by(() => {
		const id = data.escenario.id;
		return data.escenas
			.filter((e) => e.escenarioId === id)
	...
	function startEdit() {
		editingDescripcion = data.escenario.descripcion;
		editing = true;
	}
...
<section class="page">
	<a href="/proyectos/{data.proyecto.id}/escenarios" class="back">
		<ArrowLeft size={16} strokeWidth={2.2} />
		<span>Escenarios</span>
	</a>

	<header class="header">
		<MapPin size={20} strokeWidth={2} class="header-icon" />
		<h1>{data.escenario.nombre}</h1>
		<span class="apariciones">
			{escenasDelEscenario.length}
			{escenasDelEscenario.length === 1 ? 'escena' : 'escenas'}
		</span>
	</header>
	...
	<section class="block">
		...
		{:else if data.escenario.descripcion}
			<p class="descripcion">{data.escenario.descripcion}</p>
		{:else}
			<p class="descripcion-vacia">
				El script no incluye una descripción de este escenario.
			</p>
		{/if}
	</section>

	{#if escenasDelEscenario.length > 0}
		<section class="block">
			<h2>Escenas que ocurren aquí</h2>
			<ul class="escenas">
				{#each escenasDelEscenario as e (e.id)}
					<a
						class="escena-pill"
